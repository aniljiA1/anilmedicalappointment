    ...
    <div className="min-h-screen bg-surface-950 flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-sm relative">
        <div className="text-center mb-8">
          ...
          <h1 className="font-display text-2xl font-bold text-white">MediCare AI</h1>
        ...
        <div className="card p-5 lg:p-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button type="submit" disabled={loading}
              className="w-full py-2.5 rounded-xl bg-brand-500 hover:bg-brand-400 text-white font-medium text-sm transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
              {loading ? (
                <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Signing in...</>
              ) : 'Sign in'}
            </button>
          </form>
          <p className="text-center text-xs text-white/25 mt-4">Demo: admin@medicare.com / password123</p>
        </div>
      </div>
    </div>
  )
